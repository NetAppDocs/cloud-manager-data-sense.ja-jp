...
summary: 必要に応じて、 Cloud Data Sense を停止することで、 1 つ以上の作業環境、データベース、ファイル共有グループ、 OneDrive アカウント、 SharePoint アカウントをスキャンできます。作業環境でデータセンスを使用する必要がなくなった場合は、 Cloud Data Sense インスタンスを削除することもできます。 
---
...
.手順
. [_Configuration_] ページで、をクリックします image:screenshot_gallery_options.gif["[ 詳細 ] ボタン"] OneDrive または SharePoint アカウントの行にあるボタンをクリックし、 [* OneDrive アカウントの削除 * ] または [* SharePoint アカウントの削除 * ] をクリックします。
+
