...
Cloud Data Sense BYOL ライセンスを使用している場合、スキャンするすべてのデータのサイズが容量の上限に近づいている場合やライセンスの有効期限に近づいている場合、 Cloud Manager はユーザインターフェイスに警告を表示します。次の警告が表示されます。
...
* 使用しているアカウントにマーケットプレイスサブスクリプションがある場合、データセンスは引き続き実行されますが、 PAYGO ライセンスモデルに移行します。スキャンするデータの量に基づいて料金が発生します。
* 使用しているアカウントに Marketplace サブスクリプションがない場合、 Data Sense は継続的に実行されますが、引き続き警告が表示されます。


BYOL サブスクリプションを更新すると、 Cloud Manager によってライセンスが自動的に更新されます。Cloud Manager がセキュアなインターネット接続経由でライセンスファイルにアクセスできない場合（ダークサイトにインストールされている場合など）は、手動でファイルを入手して Cloud Manager にアップロードできます。手順については、を参照してください <<Update a Cloud Data Sense BYOL license,Cloud Data Sense ライセンスを更新する方法>>。

PAYGO ライセンスに切り替えられたシステムは、自動的に BYOL ライセンスに戻されます。また、ライセンスなしで実行されていたシステムでは警告が表示されなくなり、ライセンスの有効期限が切れている間に発生したアクティビティに課金されます。
